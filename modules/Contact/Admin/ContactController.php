...
class ContactController extends AdminController
{
    ...
    public function spaceRequest(Request $request)
    {
        $this->checkPermission('contact_manage');

        $s = $request->query('s');
        $datapage = SpaceRequest::orderBy('id','DESC');
        if ($s) {
            $datapage->where(function ($query) use ($s){
                $query->where('name', 'LIKE', '%' . $s . '%')
                    ->orWhere('email','LIKE', '%' . $s . '%')
        ...
        $data = [
            ...
            'breadcrumbs' => [
                [
                    'name' => __('Space Contact Submissions'),
                    'url'  => 'admin/module/space-request'
                ],
                [
                    'name'  => __('All'),
                    'class' => 'active'
        ...
        return view('Contact::admin.space-request', $data);
    }
    ...
    public function homeRequest(Request $request)
    {
        ...
        $s = $request->query('s');
        $datapage = HomeRequest::orderBy('id','DESC');
        if ($s) {
            $datapage->where(function ($query) use ($s){
                $query->where('name', 'LIKE', '%' . $s . '%')
                    ->orWhere('email','LIKE', '%' . $s . '%')
        ...
        $data = [
            ...
            'breadcrumbs' => [
                [
                    'name' => __('Home Contact Submissions'),
                    'url'  => 'admin/module/home-request'
                ],
                [
                    'name'  => __('All'),
                    'class' => 'active'
        ...
        return view('Contact::admin.home-request', $data);
    }
    ...
    public function spaceBulkEdit(Request $request)
    {
        $this->checkPermission('contact_manage');
        ...
        if ($action == "delete") {
            foreach ($ids as $id) {
                $query = SpaceRequest::where("id", $id)->first();
                if(!empty($query)){
                    $query->delete();
                }
            }
            ...
            foreach ($ids as $id) {
                $query = SpaceRequest::where("id", $id);
                $query->update(['status' => $action]);
            }
        }
        return redirect()->back()->with('success', __('Update success!'));
    }
    public function homeBulkEdit(Request $request)
    {
        $this->checkPermission('contact_manage');
        ...
        if ($action == "delete") {
            foreach ($ids as $id) {
                $query = HomeRequest::where("id", $id)->first();
                if(!empty($query)){
                    $query->delete();
                }
            }
