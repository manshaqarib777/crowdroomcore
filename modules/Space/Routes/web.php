...
Route::group(['prefix'=>config('space.space_route_prefix'),'middleware' => ['wechat_auth']],function(){
    Route::get('/','SpaceController@index')->name('space.search'); // Search
    Route::get('/{slug}','SpaceController@detail')->name('space.detail');// Detail
     Route::get('/{slug}/detail','SpaceController@detail_for_mobile')->name('space.detail.mobile');
});
