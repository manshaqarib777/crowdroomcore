...
Route::group(['prefix'=>config('car.car_route_prefix'),'middleware' => ['wechat_auth']],function(){
    Route::get('/','CarController@index')->name('car.search'); // Search
    Route::get('/{slug}','CarController@detail')->name('car.detail');// Detail
    Route::get('/{slug}/detail','CarController@detail_for_mobile')->name('car.detail.mobile');// Detail for mobile booking
});
