...
Route::group(['prefix'=>env('EVENT_ROUTE_PREFIX','event'),'middleware' => ['wechat_auth']],function(){
    Route::get('/','EventController@index')->name('event.search'); // Search
    Route::get('/{slug}','EventController@detail')->name('event.detail');// Detail
     Route::get('/{slug}/detail','EventController@detail_for_mobile')->name('event.detail.mobile');// detail for mobile booking
});
