    ...
    <div class="panel-body">
        ...
        <!--<div class="form-group">
            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="form-group">
                        <label class="control-label">{{__("Duration (in hour)")}}</label>
                        <input type="text" name="duration" class="form-control" value="{{$row->duration}}" placeholder="{{__("Example: 3")}}">
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="form-group">
                        <label class="control-label">{{__("Event Start Time")}}</label>
                        <input type="text" name="start_time" class="form-control" value="{{$row->start_time}}" placeholder="{{__("Example: 19:00")}}">
                    </div>
                </div>
            </div>
        @endif
                ...
                @if(!empty($translation->faqs))
                    ...
                    @foreach($translation->faqs as $key=>$faq)
                        <div class="item" data-number="{{$key}}">
                            <div class="row">
                                <div class="col-md-5">
                                    <input type="text" name="faqs[{{$key}}][title]" class="form-control" value="{{$faq['title']}}" placeholder="{{__('Eg: When and where does the Event end?')}}">
                                </div>
                                <div class="col-md-6">
        ...
        @if(is_default_lang())
            <div class="form-group">
                <label class="control-label">{{__("Event Poster Image")}}</label>
                <div class="form-group-image">
                    {!! \Modules\Media\Helpers\FileHelper::fieldUpload('banner_image_id',$row->banner_image_id) !!}
                </div>
            </div>
            <div class="form-group">
                <label class="control-label">{{__("Picture Gallery")}}</label>
