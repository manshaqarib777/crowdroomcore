...
//        }
    ...
    public function wechatLogin($getData) {
        $appId ='wx093ceb9d6e17391a';
        $appSecret = 'CXdQ9e87ed5wIRQsBL2YL1kOkwQI0dECLBmz6ihH4vR';
        $openId = null;

        $redirectUri = 'https://crowdroom.y3579.com';

        if(isset($getData['redirectUri']) && $getData['redirectUri']) $redirectUri=$getData['redirectUri'];

        $url = 'https://open.weixin.qq.com/connect/oauth2/authorize';
        $params = '?appid='.$appId.'&redirect_uri='.$redirectUri.'&response_type=code&scope=snsapi_userinfo&state=#wechat_redirect';
        $fullUrl = $url . $params;
    ...
    //Use the code to retrieve the webpage authorization access_token
    public function codeExchangeToken() {
        $appId ='wx093ceb9d6e17391a';
        $appSecret = '8e131b6eaf5b53dc67c0ba91dcbb61e9';
        $openId = null;
        ...
        if (isset($_GET['code']) && $_GET['code']) {
            ...
            //if(empty($user->name) || strlen($user->name) === 0) {
                $email = trim($nickname).time().'@qq.com';
                $email = str_replace(' ', '', $email);
                
                $user->email = $email;
                $user->first_name = $nickname;
                $user->last_name = 'wechat';
                ...
                $user->state= $wechatUserInfo->province;
            ...
            $user->addMeta('social_meta_avatar', $headimgurl);
