...
(function(f) {
	...
	return (function e(t, n, r) {
					...
					if (clonedCanvas) {
						...
						clonedCanvas.getContext("2d")
							.putImageData(canvas.getContext("2d")
								.getImageData(0, 0, canvas.width, canvas.height), 0, 0);
					}
				} catch (e) {
							...
							if (documentClone.body.childNodes.length > 0) {
								...
								if (options.type === "view") {
									...
									if ((/(iPad|iPhone|iPod)/g)
										.test(navigator.userAgent) && (container.contentWindow.scrollY !== y || container.contentWindow.scrollX !== x)) {
		...
		4: [function(_dereq_, module, exports) {
			...
			var loadUrlDocument = _dereq_('./proxy')
				.loadUrlDocument;
			...
			var html2canvasExport = (typeof(document) === "undefined" || typeof(Object.create) !== "function" || typeof(document.createElement("canvas")
				.getContext) !== "function") ? function() {
		...
		11: [function(_dereq_, module, exports) {
			...
			ImageLoader.prototype.isSVG = function(src) {
				return src.substring(src.length - 3)
					.toLowerCase() === "svg" || SVGContainer.prototype.isInline(src);
			};
		...
		14: [function(_dereq_, module, exports) {
			...
			NodeContainer.prototype.parseBackgroundPosition = function(bounds, image, index, backgroundSize) {
				...
				if (isPercentage(position[0])) {
					left = (bounds.width - (backgroundSize || image)
						.width) * (parseFloat(position[0]) / 100);
				} else {
				...
				if (position[1] === 'auto') {
					...
					top = (bounds.height - (backgroundSize || image)
						.height) * parseFloat(position[1]) / 100;
				} else {
			...
			NodeContainer.prototype.hasTransform = function() {
				return this.parseTransformMatrix()
					.join(",") !== "1,0,0,1,0,0" || (this.parent && this.parent.hasTransform());
			};
			...
			function NodeParser(element, renderer, support, imageLoader, options) {
				...
				if (element === element.ownerDocument.documentElement) {
					...
					var canvasBackground = new NodeContainer(parent.color('backgroundColor')
						.isTransparent() ? element.ownerDocument.body : element.ownerDocument.documentElement, null);
        ...
        ["darken", 0.60]
			...
			function calculateCurvePoints(bounds, borderRadius, borders) {
				...
				return {
					topLeft: getCurvePoints(x + borders[3].width / 2, y + borders[0].width / 2, Math.max(0, tlh - borders[3].width / 2), Math.max(0, tlv - borders[0].width / 2))
						.topLeft.subdivide(0.5),
					topRight: getCurvePoints(x + Math.min(topWidth, width + borders[3].width / 2), y + borders[0].width / 2, (topWidth > width + borders[3].width / 2) ? 0 : trh - borders[3].width / 2, trv - borders[0].width / 2)
						.topRight.subdivide(0.5),
					bottomRight: getCurvePoints(x + Math.min(bottomWidth, width - borders[3].width / 2), y + Math.min(rightHeight, height + borders[0].width / 2), Math.max(0, brh - borders[1].width / 2), brv - borders[2].width / 2)
						.bottomRight.subdivide(0.5),
					bottomLeft: getCurvePoints(x + borders[3].width / 2, y + leftHeight, Math.max(0, blh - borders[3].width / 2), blv - borders[2].width / 2)
						.bottomLeft.subdivide(0.5),

					topLeftOuter: getCurvePoints(x, y, tlh, tlv)
						.topLeft.subdivide(0.5),
					topLeftInner: getCurvePoints(x + borders[3].width, y + borders[0].width, Math.max(0, tlh - borders[3].width), Math.max(0, tlv - borders[0].width))
						.topLeft.subdivide(0.5),
					topRightOuter: getCurvePoints(x + topWidth, y, trh, trv)
						.topRight.subdivide(0.5),
					topRightInner: getCurvePoints(x + Math.min(topWidth, width + borders[3].width), y + borders[0].width, (topWidth > width + borders[3].width) ? 0 : trh - borders[3].width, trv - borders[0].width)
						.topRight.subdivide(0.5),
					bottomRightOuter: getCurvePoints(x + bottomWidth, y + rightHeight, brh, brv)
						.bottomRight.subdivide(0.5),
					bottomRightInner: getCurvePoints(x + Math.min(bottomWidth, width - borders[3].width), y + Math.min(rightHeight, height + borders[0].width), Math.max(0, brh - borders[1].width), brv - borders[2].width)
						.bottomRight.subdivide(0.5),
			...
			function hasText(container) {
				return container.node.data.trim()
					.length > 0;
			}
		...
		16: [function(_dereq_, module, exports) {
			...
			function Proxy(src, proxyUrl, document) {
				...
				return supportsCORS ? XHR(url) : (jsonp(document, url, callback)
		...
		20: [function(_dereq_, module, exports) {
			...
			function CanvasRenderer(width, height) {
				...
				this.taintCtx = this.document.createElement("canvas")
					.getContext("2d");
			...
			CanvasRenderer.prototype.taints = function(imageContainer) {
				if (imageContainer.tainted === null) {
					...
					try {
						...
						this.taintCtx = document.createElement("canvas")
							.getContext("2d");
			...
			CanvasRenderer.prototype.mask = function(shapes, callback, context, container) {
				...
				if (borderClip && borderClip.length) {
					...
					var maskShape = [canvasBorderCCW].concat(borderClip)
						.concat([borderClip[0]]);
			...
			CanvasRenderer.prototype.font = function(color, style, variant, weight, size, family) {
				...
				this.setFillStyle(color)
					.font = [style, variant, weight, size, family].join(" ")
					.split(",")[0];
			};
		...
		24: [function(_dereq_, module, exports) {
			...
			function SVGNodeContainer(node, _native) {
				...
				this.promise = _native ? new Promise(function(resolve, reject) {
						...
						self.image.onload = resolve;
						self.image.onerror = reject;
						self.image.src = "data:image/svg+xml," + (new XMLSerializer())
