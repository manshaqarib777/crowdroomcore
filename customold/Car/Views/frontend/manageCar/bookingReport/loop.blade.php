<tr>
    ...
    <td class="a-hidden">{{display_date($booking->created_at)}}<br>
    <span>{{$booking->first_name.' '.$booking->last_name}}</span>
            <br>
            <span>{{$booking->email}}</span><br>
            <span>{{$booking->phone}}</span><br>
            {{__('Booking订单 ID: # :number',['number'=>$booking->id])}}
            <br>Status状态: {{$booking->statusName}} | Amount Paid实付:{{format_money($booking->paid)}}
