...
@if(is_default_lang())
<div class="panel">
    <div class="panel-title"><strong>{{__("Pricing and Tickets")}}</strong></div>
    <div class="panel-body">
        @if(is_default_lang())
            <div class="row">
                <div class="col-lg-6">
                    <div class="form-group">
                        <label class="control-label">{{__("Price (set same as your regular ticket price. This will be visible on your event page. Setup your Ticket Types and Prices Below)")}}</label>
                        <input type="number" step="any" min="0" name="price" class="form-control" value="{{$row->price}}" placeholder="{{__("Event Price (regular ticket price)")}}">
                    </div>
                </div>
                <div class="col-lg-6">
                    <!-- <div class="form-group">
                        <label class="control-label">{{__("Sale Price (this helps you attract more buyers)")}}</label>
                        <input type="number" step="any" name="sale_price" class="form-control" value="{{$row->sale_price}}" placeholder="{{__("Event Tcket Sale Price")}}">
                        <span><i>{{__("If the regular price is less than the discounted price , it will show the regular price")}}</i></span>
                    </div>-->
                    
                </div>
            </div>
            <div class="form-group-item" >
                <label class="control-label">{{__('Tickets (Setup your ticket types and prices here)')}}</label>
                <div class="g-items-header">
                    <div class="row">
                        <div class="col-md-2">{{__("Code")}}</div>
                        <div class="col-md-5">{{__("Name")}}</div>
                        <div class="col-md-4">{{__('Ticket Price & Available Slots')}}</div>
                    ...
                    @if(!empty($row->ticket_types))
                        @foreach($row->ticket_types as $key=>$item)
                            <div class="item" data-number="{{$key}}">
                                <div class="row">
                                    <div class="col-md-2">
                                        <input type="text" @if(!is_default_lang()) disabled @endif name="ticket_types[{{$key}}][code]" class="form-control" value="{{$item['code']}}" placeholder="{{ __("regular_ticket") }}">
                                    </div>
                                    <div class="col-md-5">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="number" @if(!is_default_lang()) disabled @endif min="0" name="ticket_types[{{$key}}][price]" class="form-control" value="{{$item['price']}}" placeholder="{{ __("Ticket Price") }}">
                                        <lable>{{__("Number")}}</lable>
                                        <input type="number" @if(!is_default_lang()) disabled @endif min="0" name="ticket_types[{{$key}}][number]" class="form-control" value="{{$item['number']}}" placeholder="{{ __("Ticket Number(how mnany available)") }}">
                                    </div>
                                    <div class="col-md-1">
                                        @if(is_default_lang())
