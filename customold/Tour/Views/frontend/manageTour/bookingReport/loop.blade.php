<tr>
    ...
    <td class="a-hidden">
        ...
        <span>{{$booking->first_name.' '.$booking->last_name}}</span>
            ...
            {{__('Booking ID: # :number',['number'=>$booking->id])}}
            <br>Status: {{$booking->statusName}} | {{format_money($booking->paid)}}
            
    </td>
    <td class="a-hidden">{{display_date($booking->created_at)}}
    <br>
        {{__("To Join On")}} : {{display_date($booking->start_date)}} <br>
        {{__("Duration")}} : {{ $booking->getMeta("duration") ?? "1"  }} {{__("hours")}}</td>
